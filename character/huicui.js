...
const skills = {
  ...
  dcfuyue: {
    ...
    viewAs(cards2, player) {
      if (cards2.length) {
        var name = cards2[0].storage?.dcfuyue_name, nature = null;
        if (name) {
          return { name, nature };
        }
      }
    ...
    check(card) {
      const player = get.player(), name = card.storage?.dcfuyue_name;
  ...
  dczixi: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player.chooseButtonTarget({
        ...
        ai1(button) {
          const { max: max2 } = get.event();
          if (typeof button.link == "string") {
            if (max2.map((list) => list[0]).includes(button.link)) {
              return 1;
            }
            return 0;
          }
          if (max2.map((list) => list[2]).includes(button.link)) {
            return 1;
          }
          return 0;
        },
        ...
        complexSelect: true,
        complexTarget: true
      }).set("map", map).set("max", max).forResult();
